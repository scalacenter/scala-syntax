...
import scala.meta.internal.trees.{branch => _}
import scala.meta.internal.trees.{root => _}
import scala.meta.prettyprinters.Show
import scala.meta.prettyprinters.Show.{function => fn}
import scala.meta.prettyprinters.Show.{indent => i}
import scala.meta.prettyprinters.Show.{meta => m}
import scala.meta.prettyprinters.Show.{newline => n}
import scala.meta.prettyprinters.Show.{repeat => r}
import scala.meta.prettyprinters.Show.{sequence => s}
import scala.meta.prettyprinters.Show.{wrap => w}
...
object TreeSyntax {
  def apply[T <: Tree](dialect: Dialect): Syntax[T] = {
    object syntaxInstances {
      ...
      import SyntacticGroup.Literal
      import SyntacticGroup.Pat._
      import SyntacticGroup.Path
      ...
      def p(
        ...
        s(t) match {
          case Show.Meta(ig: SyntacticGroup, res) if groupNeedsParens(og, ig) =>
            s("(", res, ")")
          case res => res
        }
      }

      def kw(keyword: String): Show.Result = fn { sb =>
        ...
        val danger = {
          ...
          val underscoreThenOp = isIdentifierPart(prelast) && last == '_' && isOperatorPart(
            next
          )
      ...
      def templ(templ: Template): Show.Result = {
        ...
        if (templ.early.isEmpty &&
          ...
          templ.stats.isEmpty)
          s()
        else if (templ.inits.nonEmpty || templ.early.nonEmpty)
          s(" extends ", templ)
        else s(" ", templ)
      }
      ...
      implicit def syntaxTree[S <: Tree]: Syntax[S] = Syntax {
        ...
        case t: Quasi =>
          if (!dialect.allowUnquotes)
            throw new UnsupportedOperationException(
              s"$dialect doesn't support unquoting"
            )
          if (t.rank > 0) {
        ...
        case t: Name.Indeterminate =>
          if (guessIsBackquoted(t)) s("`", t.value, "`") else s(t.value)
        ...
        case t: Term.Super =>
          val thisqual =
            if (t.thisp.is[Name.Anonymous]) s() else s(t.thisp, ".")
          val superqual =
            if (t.superp.is[Name.Anonymous]) s() else s("[", t.superp, "]")
        ...
        case t: Term.Interpolate =>
          ...
          val zipped = parts.zip(t.args).map {
            case (part, id: Name) if !guessIsBackquoted(id) =>
              s(part, "$", id.value)
            case (part, arg) => s(part, "${", p(Expr, arg), "}")
          }
          val quote =
            if (parts.exists(s => s.contains("\n") || s.contains("\"")))
              "\"\"\""
            else "\""
        ...
        case t: Term.Xml =>
          if (!dialect.allowXmlLiterals)
            throw new UnsupportedOperationException(
              s"$dialect doesn't support xml literals"
            )
          val parts = t.parts.map { case Lit(part: String) => part }
          val zipped = parts.zip(t.args).map {
            case (part, arg) => s(part, "{", p(Expr, arg), "}")
        ...
        case t: Term.ApplyType =>
          m(SimpleExpr1, s(p(SimpleExpr, t.fun), t.targs))
        case t: Term.ApplyInfix =>
          ...
          m(
            ...
            s(
              p(InfixExpr(t.op.value), t.lhs, left = true),
              " ",
              t.op,
              t.targs,
              " ",
        ...
        case t: Term.Assign =>
          m(Expr1, s(p(SimpleExpr1, t.lhs), " ", kw("="), " ", p(Expr, t.rhs)))
        case t: Term.Return =>
          m(
            Expr1,
            s(
              kw("return"),
              if (guessHasExpr(t)) s(" ", p(Expr, t.expr)) else s()
            )
          )
        case t: Term.Throw => m(Expr1, s(kw("throw"), " ", p(Expr, t.expr)))
        case t: Term.Ascribe =>
          m(Expr1, s(p(PostfixExpr, t.expr), kw(":"), " ", t.tpe))
        case t: Term.Annotate =>
          m(Expr1, s(p(PostfixExpr, t.expr), kw(":"), " ", t.annots))
        ...
        case t: Term.Block =>
          ...
          t match {
            ...
            case Block(
                Function(
                  Term.Param(mods, name: Term.Name, None, _) :: Nil,
                  Block(stats)
                ) :: Nil
                ) =>
              m(
                SimpleExpr,
                s("{ ", name, " ", kw("=>"), " ", pstats(stats), n("}"))
              )
            case Block(
                Function(
                  Term.Param(_, _: Name.Anonymous, _, _) :: Nil,
                  Block(stats)
                ) :: Nil
                ) =>
              m(
                SimpleExpr,
                s("{ ", kw("_"), " ", kw("=>"), " ", pstats(stats), n("}"))
              )
            ...
            case _ =>
              m(
                SimpleExpr,
                if (t.stats.isEmpty) s("{}")
                else s("{", pstats(t.stats), n("}"))
              )
          }
        case t: Term.If =>
          m(
        ...
        case t: Term.Try =>
          m(
            ...
            s(
              ...
              " ",
              p(Expr, t.expr),
              if (t.catchp.nonEmpty)
                s(" ", kw("catch"), " {", r(t.catchp.map(i(_)), ""), n("}"))
              else s(""),
              t.finallyp
                .map { finallyp =>
        ...
        case t: Term.Function =>
          t match {
            ...
            case Term.Function(
                Term.Param(_, _: Name.Anonymous, decltpeOpt, _) :: Nil,
                body
                ) =>
              val param = decltpeOpt match {
                case Some(decltpe) =>
                  s(kw("("), kw("_"), kw(":"), decltpe, kw(")"))
                case None => s(kw("_"))
              }
            ...
            case Term.Function(params, body) =>
              m(
                Expr,
                s("(", r(params, ", "), ") ", kw("=>"), " ", p(Expr, body))
              )
          }
        case t: Term.PartialFunction =>
          m(SimpleExpr, s("{", r(t.cases.map(i(_)), ""), n("}")))
        case t: Term.While =>
        ...
        case t: Type.Project =>
          m(SimpleTyp, s(p(SimpleTyp, t.qual), kw("#"), t.name))
        case t: Type.Singleton =>
          m(SimpleTyp, s(p(SimpleExpr1, t.ref), ".", kw("type")))
        case t: Type.Apply =>
          m(
            SimpleTyp,
            s(
        ...
        case t: Type.Or =>
          if (!dialect.allowOrTypes)
            throw new UnsupportedOperationException(
              s"$dialect doesn't support or types"
            )
          m(
            InfixTyp("|"),
            s(
        ...
        case t: Type.Annotate =>
          m(AnnotTyp, s(p(SimpleTyp, t.tpe), " ", t.annots))
        case t: Type.Lambda =>
          m(Typ, t.tparams, " ", kw("=>"), " ", p(Typ, t.tpe))
        ...
        case t: Type.Param =>
          val mods =
            t.mods.filter(m => !m.is[Mod.Covariant] && !m.is[Mod.Contravariant])
          require(t.mods.length - mods.length <= 1)
          val variance = t.mods.foldLeft("")(
            (curr, m) =>
              if (m.is[Mod.Covariant]) "+"
              else if (m.is[Mod.Contravariant]) "-"
              else curr
          )
          ...
          val vbounds = {
            if (t.vbounds.nonEmpty && !dialect.allowViewBounds)
              throw new UnsupportedOperationException(
                s"$dialect doesn't support view bounds"
              )
        ...
        case t: Pat.Var =>
          m(
            SimplePattern,
            s(
              if (guessIsBackquoted(t.name)) s"`${t.name.value}`"
              else t.name.value
            )
          )
        ...
        case t: Pat.Bind =>
          val separator = t.rhs match {
            case Pat.SeqWildcard() =>
              if (dialect.allowAtForExtractorVarargs) s(" ", kw("@"))
                ...
                throw new UnsupportedOperationException(
                  s"$dialect doesn't support extractor varargs"
                )
            case _ =>
              s(" ", kw("@"))
          }
          m(
            Pattern2,
            s(p(SimplePattern, t.lhs), separator, " ", p(AnyPattern3, t.rhs))
          )
        ...
        case t: Pat.Xml =>
          ...
          val parts = t.parts.map { case Lit(part: String) => part }
          val zipped =
            parts.zip(t.args).map { case (part, arg) => s(part, "{", arg, "}") }
        ...
        case Pat.Typed(lhs, rhs: Lit) =>
          if (dialect.allowLiteralTypes)
            ...
            throw new UnsupportedOperationException(
              s"$dialect doesn't support literal types"
            )
        case t: Pat.Typed =>
          m(
            Pattern1,
            s(p(SimplePattern, t.lhs), kw(":"), " ", p(RefineTyp, t.rhs))
          )
        ...
        case Lit.Char(value) =>
          m(Literal, s(enquote(value.toString, SingleQuotes)))
        ...
        case t: Decl.Type =>
          s(w(t.mods, " "), kw("type"), " ", t.name, t.tparams, t.bounds)
        case t: Decl.Def =>
          s(
            w(t.mods, " "),
            kw("def"),
        ...
        case t: Defn.Trait =>
          if (dialect.allowTraitParameters || t.ctor.mods.isEmpty) {
            ...
            throw new UnsupportedOperationException(
              s"$dialect doesn't support trait parameters"
            )
          }
        case t: Defn.Object =>
          s(w(t.mods, " "), kw("object"), " ", t.name, templ(t.templ))
        case t: Defn.Def =>
          s(
            w(t.mods, " "),
            kw("def"),
        ...
        case t: Pkg =>
          if (guessHasBraces(t))
            s(kw("package"), " ", t.ref, " {", r(t.stats.map(i(_)), ""), n("}"))
        ...
        case t: Ctor.Primary =>
          s(w(t.mods, " ", t.mods.nonEmpty && t.paramss.nonEmpty), t.paramss)
        ...
        case t: Init =>
          s(
            if (t.tpe.is[Type.Singleton]) kw("this") else p(AnnotTyp, t.tpe),
            t.argss
          )
        ...
        case t: Template =>
          ...
          if (isTemplateEmpty) s()
            ...
            val pearly =
              if (!t.early.isEmpty) s("{ ", r(t.early, "; "), " } with ")
              else s()
            val pparents =
              w(r(t.inits, " with "), " ", !t.inits.isEmpty && !isBodyEmpty)
            val pbody = {
              val isOneLiner = t.stats.isEmpty || (t.stats.length == 1 && !s(
                t.stats.head
              ).toString.contains(EOL))
              (isSelfNonEmpty, t.stats) match {
                ...
                case (true, List(stat)) if isOneLiner =>
                  s("{ ", t.self, " => ", stat, " }")
                case (true, stats) =>
                  s("{ ", t.self, " =>", r(stats.map(i(_)), ""), n("}"))
              }
            }
        ...
        case _: Mod.Inline =>
          if (!dialect.allowInlineMods)
            throw new UnsupportedOperationException(
              s"$dialect doesn't support inline modifiers"
            )
          kw("inline")
        ...
        case t: Enumerator.Generator =>
          s(p(Pattern1, t.pat), " <- ", p(Expr, t.rhs))
        ...
        case t: Case =>
          ...
          val pcond = t.cond
            .map(cond => s(" ", kw("if"), " ", p(PostfixExpr, cond)))
            .getOrElse(s())
          val isOneLiner = {
      ...
      implicit def syntaxParamss: Syntax[List[List[Term.Param]]] =
        Syntax { paramss =>
          r(paramss.map(params => {
            s(
              ...
              w(
                "implicit ",
                r(params, ", "),
                params.exists(_.mods.exists(_.is[Mod.Implicit]))
              ),
              ")"
            )
          }), "")
    ...
    Syntax { (x: T) =>
      x.origin match {
        ...
        case Origin.Parsed(_, originalDialect, _)
            if dialect == originalDialect =>
          s(x.pos.text)
